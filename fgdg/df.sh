...
if [ -z "$BASH_VERSION" ]; then
    ...
    echo "Пожалуйста, используйте: sudo bash $0"
    exit 1
fi
...
check_dependencies() {
    ...
    if [ ${#missing[@]} -ne 0 ]; then
        echo -e "\033[1;33mRequired packages (${missing[*]}) are missing. Installing...\033[0m"
        if [ "$EUID" -ne 0 ]; then
             echo -e "\033[1;31mError: Please run as root to install dependencies or install them manually: sudo apt update && sudo apt install ${missing[*]} -y\033[0m"
             exit 1
        fi
...
declare -A LANG=(
    ...
    [LANG_EN]="English"
    [LANG_RU]="Русский"
    [ERROR_ROOT]="Script must be run as root / Скрипт должен быть запущен от имени root"
    ...
    case $1 in
        en)
            LANG=(
                ...
                [VIEW_LOGS]="View logs"
                [PRESS_ENTER_RETURN_MENU]="Press Enter to return to menu..."
                [REMNAWAVE_CLI]="Remnawave CLI"
                [ACCESS_PANEL]="Panel access via port 8443 (panel + node only)"
                [VIEW_LOGIN_DATA]="View panel login data"
                [LOGIN_DATA_TITLE]="Panel Login Credentials"
                [PANEL_URL]="Panel URL:"
                [ADMIN_LOGIN]="Admin Login:"
                [ADMIN_PASSWORD]="Admin Password:"
                [CREDS_FILE_NOT_FOUND]="Error: Credentials file not found."
                [LEGACY_CREDS_WARNING]="The password was only displayed once during the initial installation. Reinstall the panel or check your notes."
        ...
        ru)
            LANG=(
                ...
                [USERNAME]="Логин:"
                [PASSWORD]="Пароль:"
                [VIEW_LOGIN_DATA]="Посмотреть данные для входа в панель"
                [LOGIN_DATA_TITLE]="Данные для входа в панель"
                [PANEL_URL]="Ссылка на панель:"
                [ADMIN_LOGIN]="Логин администратора:"
                [ADMIN_PASSWORD]="Пароль администратора:"
                [CREDS_FILE_NOT_FOUND]="Ошибка: Файл с учетными данными не найден."
                [LEGACY_CREDS_WARNING]="Пароль отображался только один раз при первичной установке. Переустановите панель или проверьте свои записи."
...
view_login_data() {
    ...
    local creds_file="${DIR_REMNAWAVE}admin.creds"
    
    echo -e ""
    echo -e "${COLOR_GREEN}${LANG[LOGIN_DATA_TITLE]}${COLOR_RESET}"
    echo -e ""

    if [ -d "$dir" ] && [ -f "$dir/nginx.conf" ]; then
        local panel_domain=$(grep -B 20 "proxy_pass http://remnawave" "$dir/nginx.conf" | grep "server_name" | grep -v "server_name _" | awk '{print $2}' | sed 's/;//' | head -n 1)
        local cookie_line=$(grep -A 2 "map \$http_cookie \$auth_cookie" "$dir/nginx.conf" | grep "~*\w\+.*=")
        local cookies_random1=$(echo "$cookie_line" | grep -oP '~*\K\w+(?==)')
        local cookies_random2=$(echo "$cookie_line" | grep -oP '=\K\w+(?=")')
        
        if [ -n "$panel_domain" ] && [ -n "$cookies_random1" ] && [ -n "$cookies_random2" ]; then
            echo -e "${COLOR_YELLOW}${LANG[PANEL_URL]} ${COLOR_WHITE}https://${panel_domain}/auth/login?${cookies_random1}=${cookies_random2}${COLOR_RESET}"
        fi
    fi

    if [ -f "$creds_file" ]; then
        local username=$(grep "username=" "$creds_file" | cut -d'=' -f2)
        local password=$(grep "password=" "$creds_file" | cut -d'=' -f2)
        echo -e "${COLOR_YELLOW}${LANG[ADMIN_LOGIN]} ${COLOR_WHITE}$username${COLOR_RESET}"
        echo -e "${COLOR_YELLOW}${LANG[ADMIN_PASSWORD]} ${COLOR_WHITE}$password${COLOR_RESET}"
    else
        echo -e "${COLOR_RED}${LANG[CREDS_FILE_NOT_FOUND]}${COLOR_RESET}"
        echo -e "${COLOR_GRAY}${LANG[LEGACY_CREDS_WARNING]}${COLOR_RESET}"
    fi

    echo -e ""
    echo -ne "${COLOR_YELLOW}${LANG[PRESS_ENTER_RETURN_MENU]}${COLOR_RESET}"
    read
}
...
show_panel_node_menu() {
    ...
    echo -e "${COLOR_YELLOW}7. ${LANG[VIEW_LOGIN_DATA]}${COLOR_RESET}"
    echo -e ""
    echo -e "${COLOR_YELLOW}0. ${LANG[EXIT]}${COLOR_RESET}"
    echo -e ""
...
install_remnawave() {
    ...
    echo "username=$SUPERADMIN_USERNAME" > "${DIR_REMNAWAVE}admin.creds"
    echo "password=$SUPERADMIN_PASSWORD" >> "${DIR_REMNAWAVE}admin.creds"
    chmod 600 "${DIR_REMNAWAVE}admin.creds"
